"""
    Sewar package, copied from: https://pypi.org/project/sewar/
"""
import numpy as np
import warnings
...
def _initial_check(GT,P):
	...
	return GT.astype(np.float32),P.astype(np.float32)
...
def _rmse_sw_single (GT,P,ws):	
	errors = (GT-P)**2
	errors = uniform_filter(errors.astype(np.float32),ws)
...
def _scc_single(GT,P,win,ws):
	...
	GT_hp = generic_laplace(GT.astype(np.float32), _scc_filter)
	P_hp = generic_laplace(P.astype(np.float32), _scc_filter)
...
def msssim (GT,P,weights = [0.0448, 0.2856, 0.3001, 0.2363, 0.1333],ws=11,K1=0.01,K2=0.03,MAX=None):
	...
	mssim = np.array(mssim,dtype=np.float32)
...
def _vifp_single(GT,P,sigma_nsq):
	...
	for scale in range(1,5):
		...
		g[g<0]=0
		sv_sq[sv_sq<=EPS]=EPS
		# print(sv_sq, sigma_nsq, sv_sq+sigma_nsq)
...
def vifp(GT,P,sigma_nsq=2):
	...
	res = np.array([_vifp_single(GT[:,:,i],P[:,:,i],sigma_nsq) for i in range(GT.shape[2])])
